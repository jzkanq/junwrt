...
* Router-shared login wallpaper (2026-09-26): login background presets are saved in `/etc/config/junwrt-wallpaper` and shared across devices. A custom image is validated (PNG/JPEG/WebP/GIF, max 1.5 MiB), stored only in RAM as `/tmp/wpic`, and served at `/cgi-bin/junwrt-wpic` with `no-store` headers. The old browser-local image and choice keys are cleared and never used again. The mode survives reboot; a custom image does not because `/tmp` is volatile, so a missing image falls back to Graphite. Keep the rootfs view, WinSCP copies, controller, CGI source, installer generator, and generated installers synchronized; run `node --check` and both audits.
