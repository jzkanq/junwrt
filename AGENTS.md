...
* Ad Blocker enable repair (2026-09-25): the enable/update actions must verify the resulting status before reporting success. Enable downloads and validates the configured HTTPS list when no usable domains are installed; status counts canonical hosts entries, and attach/detach operates on the exact dnsmasq list entry. Preserve the previous list on failed updates and keep the rootfs, WinSCP deployment copies, firmware package, and generated installer synchronized.
